...
class Prediction(nn.Module):
	...
	def __init__(self, hidden_size, op_nums, input_size, dropout=0.5):
		...
		self.embedding_weight = nn.Parameter(torch.randn(1, input_size, hidden_size)) #what is this exactly
	...
	def forward(self, node_stacks, left_childs, encoder_outputs, num_pades, padding_hidden, seq_mask, mask_nums):
		...
		embedding_weight = torch.cat((embedding_weight, num_pades), dim=1)  # B x O x N  #num_pades - encoder embeddings of numbers; they are specific for each problem
		...
		num_score = self.score(leaf_input.unsqueeze(1), embedding_weight_, mask_nums) #mask nums bc we are not counting the num "embeddings" with no corresponding number- paddings"
