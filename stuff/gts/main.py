...
from .args import build_parser, configfile_parser
...
log_folder_test = os.path.join(os.path.dirname(__file__), 'logs_test')
...
#data_path = "/home/sylvi/diplomka/my_code/data_and_data_processing/svamp_paper_csvs"
board_path = os.path.join(os.path.dirname(__file__), './runs/')

def get_data_path(config, fold):
	if config.en:
		data_path = config.data_folder + config.dataset_path + f"foldEN{fold}/"
	elif fold is not None:
		data_path = config.data_folder + config.dataset_path + f"fold{fold}/"
	else:
		data_path = config.data_folder + config.dataset_path
	return data_path

def init_embedding(config, device, input_lang, logger):
	...
	elif config.embedding == 'robeczech':
		embedding = RobeczechEncoder()
	elif config.embedding == 'xlm-roberta':
		logger.info(f"xlm embedding: {config.emb_name}")
		embedding = XLMRobertaEncoder(config.emb_name, device, config.freeze_emb)
	else:
		embedding = Embedding(config, input_lang, input_size=input_lang.n_words, embedding_size=config.embedding_size,
...
def test(config, device):
	if config.full_cv:
		...
		for z in range(config.crossvalid_n):
			run_name = config.run_name + '_fold' + str(z)
			config.log_path = os.path.join(log_folder_test, run_name)
			config.model_path = os.path.join(model_folder, run_name)
			config.board_path = os.path.join(board_path, run_name)
			config.outputs_path = os.path.join(outputs_folder + "_test", run_name)

			data_path = get_data_path(config, None)
			...
			if config.results:
				config.result_path = os.path.join(result_folder, 'val_results_{}.json'.format(config.dataset_name))
			...
			# todo why the fuck it loads only 500 examples
			train_ls, dev_ls = load_raw_data(data_path, is_train=False)
			pairs_trained, pairs_tested, generate_nums, copy_nums = transfer_num(train_ls, dev_ls,
																				 config.challenge_disp)
			# generate nums - artificial symbols for parts of equations (assumed numbers) not in number<n> form
			# copy nums - number of number used in the problem
			...
			# Initialize models
			embedding = init_embedding(config, device, input_lang, logger)
			...
			# config.dataset_name = "asdiv-a" #todo delet this
			csv_file_path = os.path.join(config.outputs_path, config.dataset_name + '.csv')
			results_df.to_csv(csv_file_path, index=False)
			logger.info('Accuracy: {}'.format(sum(test_res_scores) / len(test_res_scores)))
			with open(config.outputs_path + '/score.txt', 'a') as f_out:
				f_out.write('Accuracy: {}'.format(sum(test_res_scores) / len(test_res_scores)))
...
def main(cnfg):
	...
	config = args
	#config.config = cnfg
	config = configfile_parser(config.config, config)
	...
	if config.full_cv and config.mode == 'train':
		...
		for z in range(config.crossvalid_n):
			run_name = config.run_name + '_fold' + str(z)
			data_path = get_data_path(config, z)
			if config.en:
				config.dataset = 'fold' + str(z) + 'EN'
			else:
			...
			train_ls, dev_ls = load_raw_data(data_path, is_griffiths=config.griffits)
			...
			checkpoint = get_latest_checkpoint(config.model_path, logger) #todo - here load the previous model
			...
			# Initialize models #todo - here load the model instead of initing new one
			embedding = init_embedding(config, device, input_lang, logger)
		...
		if not is_train:
			...
			data_path = get_data_path(config, 0)
			...
			logger.info('Loading Data...')
			#todo why the fuck it loads only 500 examples
			train_ls, dev_ls = load_raw_data(data_path, is_train=is_train)
			pairs_trained, pairs_tested, generate_nums, copy_nums = transfer_num(train_ls, dev_ls, config.challenge_disp)
			#generate nums - artificial symbols for parts of equations (assumed numbers) not in number<n> form
			#copy nums - number of number used in the problem
			...
			if is_train:
				...
				# Initialize models
				embedding = init_embedding(config, device, input_lang)
