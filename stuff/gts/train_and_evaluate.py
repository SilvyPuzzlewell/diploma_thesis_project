...
def generate_tree_input(target, decoder_output, nums_stack_batch, num_start, unk):
	...
	for i in range(len(target)):
		...
		if target_input[i] >= num_start:
			target_input[i] = 0
	return torch.LongTensor(target), torch.LongTensor(target_input) #target_input; if nums - 0? it's used for generating (next goals?)
...
def train_tree(config, input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
	...
	embedding.train() #this just sets to training mode?
	encoder.train()
	predict.train()
	generate.train()
	merge.train()
	...
	embedded = None
	orig_idx = None
	if config.embedding == 'bert' or config.embedding == 'roberta' or config.embedding == 'robeczech'\
			or config.embedding == 'xlm-roberta':
	...
	# encoder outputs - hidden states for each input token i assume; problem output - final embedding/or combination of first and final?
	# encoder_outputs, problem_output = encoder(input_var, input_length)

	# sequence mask for attention #do not pay attention to padding
	seq_mask = []
	max_len = max(input_length)
	for i in input_length:
		seq_mask.append([0 for _ in range(i)] + [1 for _ in range(i, max_len)]) #inputs are padded to the max input size in batch
	seq_mask = torch.BoolTensor(seq_mask)

	if USE_CUDA:
		seq_mask = seq_mask.cuda()

	# Prepare input and output variables; i assume problem output is the last hidden state of rnn; ie. i need it to create the root node and i also need the remaining hidden states for attention
	...
	for t in range(max_target_length):                     #stop producing tokens by force once max target length reached?
		num_score, op, current_embeddings, current_context, current_nums_embeddings = predict(
			node_stacks, left_childs, encoder_outputs, all_nums_encoder_outputs, padding_hidden, seq_mask, num_mask)
		# outputs = score for numbers, score for operators, node embeddings, node context vectors, nums embeddings
		# all_leafs.append(p_leaf)
		outputs = torch.cat((op, num_score), 1) #decode the most likely node token?
		...
		left_child, right_child, node_label = generate(current_embeddings, generate_input, current_context) #subgoal embeddings?
		left_childs = []
		for idx, l, r, node_stack, i, o in zip(range(batch_size), left_child.split(1), right_child.split(1),
			...
			if i < num_start:  #if target isn't number, generate new subgoals
				node_stack.append(TreeNode(r))
				node_stack.append(TreeNode(l, left_flag=True))
				o.append(TreeEmbedding(node_label[idx].unsqueeze(0), False))
			else:
				current_num = current_nums_embeddings[idx, i - num_start].unsqueeze(0) #target(i) is a number (>=start_num)
				while len(o) > 0 and o[-1].terminal:
					sub_stree = o.pop()
					op = o.pop()
					current_num = merge(op.embedding, sub_stree.embedding, current_num)
				o.append(TreeEmbedding(current_num, True))
			if len(o) > 0 and o[-1].terminal: #for terminal node, append its embedding to left childs?
...
def evaluate_tree(config, input_batch, input_length, generate_nums, embedding, encoder, predict, generate, merge, input_lang, output_lang, num_pos,
	...
	embedded = None
	orig_idx = None
	if config.embedding == 'bert' or config.embedding == 'roberta' or config.embedding == 'robeczech' or config.embedding == 'xlm-roberta':
		contextual_input = index_batch_to_words([input_batch], [input_length], input_lang)
		input_seq1, input_len1 = embedding(contextual_input)
		input_seq1 = input_seq1.transpose(0,1)
		embedded, input_length, orig_idx = sort_by_len(input_seq1, input_len1, gpu_init_pytorch(config.gpu))
