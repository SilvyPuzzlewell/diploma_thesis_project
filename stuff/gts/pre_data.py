...
import pandas as pd
import nltk
from ..data_and_data_processing.data_loading_wrapper import get_griffiths_mawps_train, get_griffiths_mawps_test

nltk.download('punkt')
import pdb

#from data_and_data_processing.data_loading_wrapper import get_griffiths_mawps_train
...
class Lang:
	...
	def build_output_lang_for_tree(self, generate_num, copy_nums):  # build the output lang vocab and dict
		...
		for i in range(copy_nums):
			curnum = "N" + str(i)
			if curnum not in self.index2word:
				self.index2word.append(curnum)
		if "UNK" not in self.index2word:
...
def load_raw_data(data_path, is_train = True, is_griffiths = False):  # load the data_and_data_processing to list(dict())
	data_path = os.path.join(os.path.dirname(__file__), data_path)
	train_ls = None
	if is_train:
		if is_griffiths:
			train_df = get_griffiths_mawps_train()
		else:
	...
	if is_griffiths:
		dev_df = get_griffiths_mawps_test()
	else:
...
def transfer_num(train_ls, dev_ls, chall=False):  # transfer num into "NUM"
	...
	if train_ls != None:
		...
		for d in train_ls:
			...
			opz = ['+', '-', '*', '/']
			idxs = []
			for s in range(len(seg)): #seg - tokenized body + question; replaces number<n> with NUM and adds position of number to idxs
				if len(seg[s]) >= 7 and seg[s][:6] == "number" and seg[s][6] in numz:
					input_seq.append("NUM")
					idxs.append(s)
				else:
					input_seq.append(seg[s])
			if copy_nums < len(nums): #what is the point of copy_nums
				copy_nums = len(nums)

			out_seq = []
			for e1 in equation:
				if len(e1) >= 7 and e1[:6] == "number": #replace numbers in equation by N<n>
					out_seq.append('N'+e1[6:])
				elif e1 not in opz:                     #some unknown symbol in equation; treat it as number
...
def prepare_data(config, logger, pairs_trained, pairs_tested, trim_min_count, generate_nums, copy_nums, input_lang=None, output_lang=None, tree=False):
	...
	if config.embedding == 'bert' or config.embedding == 'roberta':
		for pair in pairs_tested:                                    #adds test data_and_data_processing to vocab
			if not tree:
				input_lang.add_sen_to_vocab(pair[0])
			elif pair[-1]:
				input_lang.add_sen_to_vocab(pair[0])
		...
		for pair in pairs_trained:
			...
			for word in pair[1]: # For each token in equation
				...
				if word not in output_lang.index2word: # If token is not in output vocab
					token_in_lang = False
					for i, j in enumerate(pair[2]): #pair 2 = quantities mentioned in the problem
						if j == word:				#if word in equation is number in the quantities
							temp_num.append(i)      #appned it's index in the quantities list to "temp_num"

				if not token_in_lang and len(temp_num) != 0: # token not in output lang and a number in the quantities list
					num_stack.append(temp_num)               #append the indices of the number in quantities list
				if not token_in_lang and len(temp_num) == 0: # token not in output lang and not a number in the quantities list
					num_stack.append([_ for _ in range(len(pair[2]))]) #apparently all indices in the quantities list?
			...
			#train pairs - indices of input problem tokens in land; len of that sequence; the same for output; quantities mentioned in the problem
			#indices of those quantities in the input cell
	...
	for pair in pairs_tested:
		...
		for word in pair[1]:
			...
			if not token_in_lang and len(temp_num) != 0:
...
def prepare_train_batch(pairs_to_batch, batch_size):
	...
	for batch in batches:
		batch = sorted(batch, key=lambda tp: tp[1], reverse=True) #sorted by length (num tokens) of input
		input_length = []
		output_length = []
